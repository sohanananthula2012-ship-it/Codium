...
project-root/
  ...
  │   │   └── ide/           # Daytona Code Editor: TitleBar, ActivityBar, Sidebar,
  │   │                      # FileTree, SandboxPanel, SearchPanel, SourceControl,
  │   │                      # EditorArea, TabBar, Breadcrumbs, MonacoEditor, Terminal,
  │   │                      # StatusBar, CommandPalette
  │   ├── hooks/             # Custom React hooks (use-ide-state is a context provider,
  │   │                      # use-keyboard-shortcut binds global key combos)
  │   ├── i18n/              # i18n runtime: config.ts (entry) + util.ts (helpers)
  │   ├── lib/               # Utility functions and libraries
  │   │                      # (ide-seed.ts seeds the mocked file tree, file contents,
  │   │                      # sandbox info, source control, search, and command palette)
  │   ├── pages/             # Application pages (each page in its own subdirectory)
  │   │   └── ide/           # The Daytona Code Editor page (the home page re-exports it)
  │   ├── App.tsx            # Main app component, sets up route providers
  │   ├── router.tsx         # Router config, sets up routing
  │   ├── main.tsx           # Entry point for the React app
  │   └── index.css          # Global styles + dark-only design tokens
